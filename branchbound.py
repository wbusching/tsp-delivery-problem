...
def branch_and_bound(g, sub_cycle=None, current_min=float("inf")):
    ...
    for v in g.nodes():
        if v not in sub_cycle:
            unused_nodes.append((g[sub_cycle[-1]][v]['weight'], v))   # store unused nodes as a tuple: (distance from current, node #)

    # We sort them by the distance from the "current node" -- the last node in sub_cycle.
    unused_nodes = sorted(unused_nodes)

    for (d, v) in unused_nodes:  # d = distance from current node to unused node; v = node number
        assert v not in sub_cycle
        extended_subcycle = list(sub_cycle)
        extended_subcycle.append(v)  # consider an extended_subcycle with v added to it
        # determine whether the lower bound, given this extended subcycle, is already higher than the current min
